...
single global number.** Expected shape of the report:

- `src/solver/` and `src/app/undo.rs` should be near 100%. They're
  pure logic, exhaustively unit-tested, and any drop here is a real
  regression worth investigating.
- `src/mazes/` should be high (parsers and generators are easy to
  test).
- `src/app/state.rs`, `app/canvas.rs`, `app/side_panel.rs`, and
  `app/top_bar.rs` will sit much lower — they're driven by egui
  events, mouse clicks, and OS file dialogs. Unit-testing them
  meaningfully would require a headless egui harness (we don't
  have one).
- `src/platform/fileio.rs` is essentially untestable from `cargo
  test` because it talks to OS file pickers or the DOM.
